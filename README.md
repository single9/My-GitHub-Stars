...
A terminal UI, desktop GUI, and web app for browsing, searching, and categorising your GitHub starred repositories.
...
- **Three UI modes** — classic terminal (TUI), native desktop window (GUI, powered by Dioxus), or local web app (Web)
...
**TUI / Web build** — no system libraries required (SQLite is bundled).
...
# Web server
cargo install --path . --features web --no-default-features
```
...
# Local web app  (opens browser at http://127.0.0.1:3000)
cargo run --features web --no-default-features
```
...
The web mode starts a lightweight HTTP server on `http://127.0.0.1:3000` and opens your default browser automatically.
It provides a read-only browse and search interface backed by the same local SQLite database.

> **Note:** Sync and authentication are not available in web mode.
> Run the TUI or GUI first to authenticate and sync your stars, then use web mode for browsing.
...
```
src/
├── main.rs          Entry point — selects TUI, GUI, or Web based on feature flag
...
└── web/             Local web server (axum, compiled with --features web)
    ├── mod.rs       HTTP server, REST API handlers, run() entry point
    └── frontend.html  Embedded single-page app (dark GitHub-style UI)
```
